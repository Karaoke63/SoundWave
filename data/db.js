...
const DB = {
  ...
  // ── Синхронизация с AdminPanel (localStorage) ───────────
  // Вызывается при загрузке страницы — перекрывает дефолтные данные
  // данными из админки если они есть
  _loadFromStorage() {
    try {
      const raw = localStorage.getItem("sw_admin_db");
      if (!raw) return;
      const saved = JSON.parse(raw);
      if (saved.artists) this.artists = saved.artists;
      if (saved.albums)  this.albums  = saved.albums;
      if (saved.tracks)  this.tracks  = saved.tracks;
      if (saved.genres)  this.genres  = saved.genres;
    } catch(e) { console.warn("DB sync error", e); }
  },
};

// Загружаем данные из админки сразу при подключении скрипта
DB._loadFromStorage();

// Слушаем изменения из других вкладок (админка → сайт в реальном времени)
window.addEventListener("storage", (e) => {
  if (e.key !== "sw_admin_db") return;
  DB._loadFromStorage();
  // Если страница уже открыта — обновляем текущий вид
  if (typeof Router !== "undefined" && typeof Pages !== "undefined") {
    const cur = document.querySelector(".page.active")?.id?.replace("page-","");
    if (cur && Pages[cur]) Pages[cur].render({});
    if (typeof refreshSidebarPlaylists === "function") refreshSidebarPlaylists();
  }
});
